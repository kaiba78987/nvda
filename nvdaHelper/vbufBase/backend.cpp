...
void VBufBackend_t::initialize() {
	...
	if (!SendMessageCallback((HWND)UlongToHandle(rootDocHandle),wmRenderThreadInitialize,(WPARAM)this,0,callback,NULL))
		return;
	MSG msg;
...
void VBufBackend_t::terminate() {
	if(runningBackends.count(this)>0) {
		...
		if (!SendMessageCallback((HWND)UlongToHandle(rootDocHandle),wmRenderThreadTerminate,(WPARAM)this,0,callback,NULL))
			return;
		MSG msg;
		while (GetMessage(&msg, NULL, WM_QUIT, WM_QUIT) > 0);
		LOG_DEBUG(L"Message sent, unregistering hook");
